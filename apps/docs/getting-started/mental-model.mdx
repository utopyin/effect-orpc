...
If the Node bridge is installed, `effect-orpc` carries current `FiberRefs` through native oRPC continuations and merges them into the next Effect boundary. Use the Node bridge guide when request-local Effect state must cross those split boundaries in Node.

## Guides

Guides cover concrete integrations and examples such as Hono, OpenTelemetry, the Node fiber context bridge, and testing.

## Next step

Continue to [Effect procedures](/capabilities/effect-procedures).
